import { useState, useRef, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { EraserIcon, Headphones, HeadphoneOff } from "lucide-react";
import Image from "next/image";
import { CHAT_HEADER, CLEAR_BUTTON_TEXT } from "@/configuration/ui";
import { AI_NAME } from "@/configuration/identity";

export const AILogo = () => (
  <div className="w-12 h-12 relative">
    <Image src="/big_green_junimo.gif" alt={AI_NAME} width={48} height={48} />
    <div className="w-2 h-2 rounded-full bg-green-500 absolute -bottom-0.5 -right-0.5"></div>
  </div>
);

export default function ChatHeader({
  clearMessages,
}: {
  clearMessages: () => void;
}) {
  const [isMusicOn, setIsMusicOn] = useState(false);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // Play audio when the component mounts
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = 0.30; // 🎵 Set volume to 50% (range: 0.0 - 1.0)
      audioRef.current.play().catch(() => console.warn("Autoplay blocked"));
    }
  }, []);

  const toggleMusic = () => {
    if (audioRef.current) {
      if (isMusicOn) {
        audioRef.current.pause();
      } else {
        audioRef.current.play();
      }
    }
    setIsMusicOn((prev) => !prev);
  };

  return (
    <div className="relative">
      {/* Background Music */}
      <audio ref={audioRef} src="/background.mp3" loop />

      {/* Chat Header */}
      <div className="z-10 flex justify-center items-center fixed top-0 w-full p-5 bg-white shadow-[0_10px_15px_-3px_rgba(255,255,255,1)]">
        <div className="flex w-full">
          <div className="flex-0 w-[100px]"></div>
          <div className="flex-1 flex justify-center items-center gap-2">
            <AILogo />
            <p>{CHAT_HEADER}</p>
          </div>
          <div className="flex-0 w-[100px] flex justify-end items-center gap-x-4">
            <Button
              onClick={clearMessages}
              className="gap-2 shadow-sm"
              variant="outline"
              size="sm"
            >
              <EraserIcon className="w-4 h-4" />
              <span>{CLEAR_BUTTON_TEXT}</span>
            </Button>

            <Button
              onClick={toggleMusic}
              className="gap-2 shadow-sm"
              variant="outline"
              size="sm"
            >
              {isMusicOn ? (
                <Headphones className="w-4 h-4" />
              ) : (
                <HeadphoneOff className="w-4 h-4" />
              )}
              <span>{isMusicOn ? "Music On" : "Music Off"}</span>
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
}
